document.addEventListener("DOMContentLoaded", function () {
  const form = document.getElementById("form-perfil");

  if (form) {
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      const newPass = $("#newPass");
      const confirmPass = $("#confirmPass");
      const token = document.querySelector('meta[name="csrf-token"]').content;
      if (newPass.val().trim() !== confirmPass.val().trim()) {
        Swal.fire({
          icon: "error",
          title: "Oops...",
          text: "Las contraseñas no coinciden!",
        });
        return;
      }

      $("#newPass, #confirmPass").on("input", function () {
        if ($(this).val().trim() !== "") {
          $(this)
            .removeClass("is-invalid")
            .parent()
            .next(".message")
            .css("display", "none");
        }
      });

      if (newPass.val().trim() === "") {
        newPass
          .addClass("is-invalid")
          .parent()
          .next(".message")
          .css("display", "block")
          .text("La contraseña es obligatoria");
      }

      if (confirmPass.val().trim() === "") {
        confirmPass
          .addClass("is-invalid")
          .parent()
          .next(".message")
          .css("display", "block")
          .text("La confirmación de la contraseña es obligatoria");
      }

      if (newPass.val().trim() != "" && confirmPass.val().trim() != "") {
        Swal.fire({
          title: "Cambiar contraseña",
          html: `
            <div class="form-group-icon">
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="#000000" fill="none">
                    <path d="M13.4082 16.6682C13.4082 16.6682 14.0332 16.6682 14.6582 18.0015C14.6582 18.0015 16.6435 14.6682 18.4082 14.0015" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M16.9434 7.00146H16.9524" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M11.9434 7.00146H11.9524" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M6.94336 7.00146H6.95234" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M7.89193 11.9685H5.00845C3.34693 11.9685 2 10.6254 2 8.96851V4.99854C2 3.34168 3.34693 1.99854 5.00845 1.99854H18.9916C20.6531 1.99854 22 3.34168 22 4.99854V9.12944" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M21.9996 16.0015C21.9996 12.6878 19.3057 10.0015 15.9827 10.0015C12.6597 10.0015 9.96582 12.6878 9.96582 16.0015C9.96582 19.3152 12.6597 22.0015 15.9827 22.0015C19.3057 22.0015 21.9996 19.3152 21.9996 16.0015Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                </svg>
              </span>
              <input type="password" class="form-control" id="actualPass" name="actual_password" placeholder="Ingresa tu contraseña actual">
            </div>
            <span class="message"></span>
          `,
          showCancelButton: true,
          confirmButtonText: "Cambiar",
          cancelButtonText: "Cancelar",
          preConfirm: () => {
            const actuallyPass = $("#actualPass");
            if (actuallyPass.val().trim() === "") {
              actuallyPass.addClass("is-invalid");
              Swal.showValidationMessage("La contraseña es obligatoria.");
            } else {
              return {
                actual_password: actuallyPass.val().trim(),
              };
            }
          },
        }).then((result) => {
          if (result.isConfirmed) {
            $.ajax({
              url: form.action,
              type: form.method,
              data: {
                _token: token,
                actual_password: result.value.actual_password,
                nueva_password: newPass.val().trim(),
              },
              success: function (response) {
                var data = JSON.parse(response);
                if (data.status == "success") {
                  Swal.fire({
                    icon: data.status,
                    title: data.title,
                    text: data.message,
                  }).then((result) => {
                    if (result.isConfirmed) {
                      window.location.href = data.redirect;
                    }
                  });
                  form.reset();
                } else {
                  Swal.fire({
                    icon: data.status,
                    title: data.title,
                    text: data.message,
                  });
                }
              },
            });
          }
        });
      }
    });
  }
});
